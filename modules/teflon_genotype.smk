rule teflon_genotype_individual :
	...
	params:
		...
		frequencyLoThresh = config["PARAMS"]["GENOTYPE"]["SAMPLE"]["THRESHOLD_ABSENCE"],
		frequencyHiThresh = config["PARAMS"]["GENOTYPE"]["SAMPLE"]["THRESHOLD_PRESENCE"]
	resources:
		mem_mb=get_mem_mb
	...
	run:# delete everything so we can re-run things
		...
		if (check_value(config["PARAMS"]["GENOTYPE"]["SAMPLE"]["THRESHOLD_ABSENCE"])) :
			cmd = cmd + ("-flt {params.frequencyLoThresh} ")
		if (check_value(config["PARAMS"]["GENOTYPE"]["SAMPLE"]["THRESHOLD_PRESENCE"])) :
