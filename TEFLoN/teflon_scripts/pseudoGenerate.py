...
def pseudoMapMake(the_list, val):
    return [i+1 for i in range(len(the_list)) if the_list[i] != val]
...
def removeBedPos(bedFile, fasta, prep_MP_DIR,prep_TF_DIR, pre):
    ...
    with open(outFILE, "w") as fOUT1, open(os.path.join(prep_TF_DIR,pre+".genomeSize.txt"), 'w') as fOUT2:
        ...
        print("Writing pseudospace to refspace conversion map:",os.path.join(prep_TF_DIR,pre+".pseudo2ref.txt"))
        print("Writing refspace to pseudospace conversion map:",os.path.join(prep_TF_DIR,pre+".ref2pseudo.txt"))
    ...
    print("Dumping json...")
    with open(os.path.join(prep_TF_DIR,pre+".pseudo2ref.txt"),"w") as psMap:
        json.dump(pseudoMap,psMap)
    print(os.path.join(prep_TF_DIR,pre+".pseudo2ref.txt"), "dumped!")
