...
def worker(task_q, result_q, params):
    while True:
        try:
            ...
            #unpack parameters
            genoDir, tmpDir, tmpUnion, exePATH, union, samples, qual, hierarchy, label, l2, annotation = params
            tmp=[]
            ct=1
            for ID in siteID:
                flag=0
                ...
                samFILE = wb.wb_portal(ID, union, samples, tmpDir, tmpUnion, exePATH, qual, "firstPass")
                #TEST samFILE
                # with open("samFILEdico", 'a') as ffile:
                #     ffile.write(str(samFILE)+"\n")
                ...
                if flag==1: #position estimates were refined, recount
                    # 1. write sam files for all samples
                    samFILE = wb.wb_portal(ID, union, samples, tmpDir, tmpUnion, exePATH, qual, "refine")
                    # 2. count support and non support reads for each event
                    counts = cr.countReads_portal(ID, union, samples, tmpDir, samFILE, hierarchy, label, l2, annotation) # counts = [[#p(sample1),#a(sample1),#NA(sample1)],[#p(sampleN),#a(sampleN),#NA(sampleN)]]
...
def main():
    ...
    # reduce search space
    samples[0][0]=rss.reduceSearchSpace_portal(union,samples,tmpDir,tmpUnion,exeSAM,qual,nProc)
    ...
    siteID = list(range(len(union)))
    task_q = mp.JoinableQueue()
    result_q = mp.Queue()
    params=[genoDir, tmpDir, tmpUnion, exeSAM, union, samples, qual, hierarchy, label, l2, annotation]
